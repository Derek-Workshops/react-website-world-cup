...
const TeamCrest: React.FC<{ badge?: string; flag: string; name: string }> = ({ badge, flag, name }) =>
  badge ? (
    <img src={badge} alt={name} loading="lazy" className="w-12 h-12 object-contain" />
  ) : (
  ...
  <div className="bg-white/5 border border-white/10 hover:border-[#f5a623]/40 rounded-2xl p-5 transition-all duration-200">
    ...
    <div className="flex items-center gap-4">
      ...
      <div className="flex-1 flex flex-col items-center gap-2">
        <TeamCrest badge={match.homeBadge} flag={match.homeFlag} name={match.homeTeam} />
      ...
      {/* Away */}
      <div className="flex-1 flex flex-col items-center gap-2">
        <TeamCrest badge={match.awayBadge} flag={match.awayFlag} name={match.awayTeam} />
